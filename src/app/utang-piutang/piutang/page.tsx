...
export default function PiutangPage() {
  ...
  const [tipe, setTipe] = useState<TipePiutang>('piutang_usaha')
  ...
  function resetForm() {
    ...
    setNominal(''); setKlien(''); setDeskripsi(''); setKasId('tidak'); setTipe('piutang_usaha')
  ...
  async function handleSimpan() {
    ...
    const res = await tambahPiutang({
      ...
      nominal: Number(nominal), klien, deskripsi,
      kas_id: kasId !== 'tidak' ? kasId : null, tipe,
    })
    setIsErr(!!res.error)
  ...
  const inp = { className: "w-full border rounded px-3 py-2 text-sm focus:outline-none", style: { borderColor: 'var(--border)', background: 'var(--card-bg)', color: 'var(--text)' } }
    ...
    <div className="space-y-5 max-w-6xl mx-auto">
      ...
      {showForm && (
        <div className="card p-6">
          ...
          {msg && <div className={`mb-4 p-3 rounded text-sm ${isErr ? 'bg-red-50 text-red-600 border border-red-200' : 'bg-green-50 text-green-700 border border-green-200'}`}>{msg}</div>}
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium mb-1" style={{ color: 'var(--text)' }}>Tanggal Awal *</label>
                <input type="date" value={tanggalAwal} onChange={e => setTanggalAwal(e.target.value)} {...inp} />
              </div>
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <input type="checkbox" id="jt" checked={pakeJatuhTempo} onChange={e => setPakeJatuhTempo(e.target.checked)} className="w-4 h-4 rounded" style={{ accentColor: 'var(--brand)' }} />
                  <label htmlFor="jt" className="text-sm font-medium" style={{ color: 'var(--text)' }}>Ada Jatuh Tempo</label>
                </div>
                {pakeJatuhTempo && <input type="date" value={jatuhTempo} onChange={e => setJatuhTempo(e.target.value)} {...inp} />}
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
              ...
              <div>
                ...
                <input type="text" value={klien} onChange={e => setKlien(e.target.value)} placeholder="Nama klien" {...inp} />
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium mb-1" style={{ color: 'var(--text)' }}>Deskripsi *</label>
              <textarea value={deskripsi} onChange={e => setDeskripsi(e.target.value)} placeholder="Keterangan piutang..." rows={2} className="w-full border rounded px-3 py-2 text-sm focus:outline-none resize-none" style={{ borderColor: 'var(--border)', background: 'var(--card-bg)', color: 'var(--text)' }} />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium mb-1" style={{ color: 'var(--text)' }}>Tipe Piutang</label>
                <select value={tipe} onChange={e => setTipe(e.target.value as TipePiutang)} {...inp}>
                  <option value="piutang_usaha">Piutang Usaha</option>
                  <option value="tagihan_berulang">Tagihan Berulang</option>
                  <option value="cicilan">Cicilan</option>
                  <option value="kasbon">Kasbon</option>
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium mb-1" style={{ color: 'var(--text)' }}>Catat di Buku Kas?</label>
      ...
      <div className="card p-6">
        <div className="flex items-center justify-between mb-4 pb-3" style={{ borderBottom: '1px solid var(--border)' }}>
          <h2 className="text-base font-bold" style={{ color: 'var(--text)' }}>Daftar Piutang ({filtered.length})</h2>
          <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Cari klien..." className="border rounded px-3 py-1.5 text-sm w-52 focus:outline-none" style={{ borderColor: 'var(--border)', background: 'var(--card-bg)', color: 'var(--text)' }} />
        </div>
        ...
        {loading ? (
          ...
          <div className="py-10 text-center">
            ...
            <p className="text-sm" style={{ color: 'var(--text-muted)' }}>{search ? 'Tidak ditemukan' : 'Belum ada piutang.'}</p>
          </div>
        ) : (
          <>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr style={{ color: 'var(--text-muted)', borderBottom: '1px solid var(--border)' }}>
                    ...
                    <th className="pb-2 text-left font-semibold pr-3 hidden sm:table-cell">Tipe</th>
                  ...
                  {paginated.map(p => (
                    <tr key={p.id} style={{ borderBottom: '1px solid var(--border)' }}>
                      <td className="py-3 pr-3">
                        <button onClick={() => handleToggleStatus(p)} className="text-xs px-2 py-0.5 rounded-full font-medium cursor-pointer" style={{ background: p.status === 'lunas' ? 'rgba(34,197,94,0.1)' : 'rgba(239,68,68,0.1)', color: p.status === 'lunas' ? '#22c55e' : '#ef4444' }}>
                      ...
                      <td className="py-3 pr-3 whitespace-nowrap" style={{ color: 'var(--text)' }}>
                        ...
                        {p.jatuh_tempo && <p className="text-xs" style={{ color: 'var(--text-muted)' }}>JT: {new Date(p.jatuh_tempo).toLocaleDateString('id-ID')}</p>}
                      </td>
                      ...
                      <td className="py-3 pr-3 hidden sm:table-cell">
                        <span className="text-xs px-2 py-0.5 rounded-full font-medium" style={{ background: 'rgba(34,197,94,0.1)', color: '#22c55e' }}>
                          {(p.tipe ?? 'piutang_usaha').replace(/_/g, ' ')}
                        </span>
                      </td>
                      ...
                      <td className="py-3 text-center">
                        <button onClick={() => handleHapus(p.id)} className="text-xs px-3 py-1 rounded border font-semibold" style={{ borderColor: 'rgba(220,38,38,0.4)', color: '#dc2626' }}>Hapus</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between mt-4 pt-4" style={{ borderTop: '1px solid var(--border)' }}>
                <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="px-4 py-2 rounded text-sm border disabled:opacity-50" style={{ borderColor: 'var(--border)', color: 'var(--text-muted)' }}>← Sebelumnya</button>
                <span className="text-sm" style={{ color: 'var(--text-muted)' }}>Halaman {currentPage} dari {totalPages}</span>
                <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="px-4 py-2 rounded text-sm border disabled:opacity-50" style={{ borderColor: 'var(--border)', color: 'var(--text-muted)' }}>Selanjutnya →</button>
              </div>
            )}
          </>
        )}
