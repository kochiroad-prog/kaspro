...
export default function UtangPage() {
  ...
  const [tipe, setTipe] = useState<TipeUtang>('hutang_usaha')
  ...
  function resetForm() {
    ...
    setNominal(''); setKlien(''); setDeskripsi(''); setKasId('tidak'); setTipe('hutang_usaha')
    ...
    <div className="space-y-5 max-w-6xl mx-auto">
        ...
        <div className="card p-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea value={deskripsi} onChange={e => setDeskripsi(e.target.value)} placeholder="Keterangan utang..." rows={2} className="w-full border rounded px-3 py-2 text-sm focus:outline-none resize-none" style={{ borderColor: 'var(--border)', background: 'var(--card-bg)', color: 'var(--text)' }} />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium mb-1" style={{ color: 'var(--text)' }}>Tipe Utang</label>
                <select value={tipe} onChange={e => setTipe(e.target.value as TipeUtang)} {...inp}>
      ...
      <div className="card p-6">
        <div className="flex items-center justify-between mb-4 pb-3" style={{ borderBottom: '1px solid var(--border)' }}>
          <h2 className="text-base font-bold" style={{ color: 'var(--text)' }}>Daftar Utang ({filtered.length})</h2>
        ...
        {loading ? (
          ...
          <div className="py-10 text-center">
            ...
            <p className="text-sm" style={{ color: 'var(--text-muted)' }}>{search ? 'Tidak ditemukan' : 'Belum ada utang.'}</p>
          </div>
        ) : (
          <>
                  ...
                  {paginated.map(u => (
                    <tr key={u.id} style={{ borderBottom: '1px solid var(--border)' }}>
                      <td className="py-3 pr-3">
                        <button onClick={() => handleToggleStatus(u)} className="text-xs px-2 py-0.5 rounded-full font-medium cursor-pointer" style={{ background: u.status === 'lunas' ? 'rgba(34,197,94,0.1)' : 'rgba(239,68,68,0.1)', color: u.status === 'lunas' ? '#22c55e' : '#ef4444' }}>
                      ...
                      <td className="py-3 pr-3 whitespace-nowrap" style={{ color: 'var(--text)' }}>
                        ...
                        {u.jatuh_tempo && <p className="text-xs" style={{ color: 'var(--text-muted)' }}>JT: {new Date(u.jatuh_tempo).toLocaleDateString('id-ID')}</p>}
                      </td>
                      ...
                      <td className="py-3 pr-3 hidden sm:table-cell">
                        <span className="text-xs px-2 py-0.5 rounded-full font-medium" style={{ background: 'var(--brand-light)', color: 'var(--brand)' }}>
                          {(u.tipe ?? 'hutang_usaha').replace(/_/g, ' ')}
                        </span>
                      </td>
                      ...
                      <td className="py-3 text-center">
                        <button onClick={() => handleHapus(u.id)} className="text-xs px-3 py-1 rounded border font-semibold" style={{ borderColor: 'rgba(220,38,38,0.4)', color: '#dc2626' }}>Hapus</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
